...
export class BattleScene extends Phaser.Scene{
    ...
    playerSkillEffect(){
        ...
        if (this.playerAttack.missed){
            ...
            const fx = this.add.sprite(this.defender.x, this.defender.y, `anim_${skillId}`);
    ...
    enemySkillEffect(){
        if(this.enemyAttack == null) {
            console.warn("Enemy attack is null, skipping state updates.");
            return;
        };
        ...
        if (this.enemyAttack.missed){
            ...
            const fx = this.add.sprite(this.attacker.x, this.attacker.y, `anim_${skillId}`);
            fx.setDisplaySize(fx.displayWidth/1.5, fx.displayHeight/1.5)
            .setOrigin(0.5,1);
...
function updatePlayerState(scene){
    if (scene.playerState.hypno && !scene.stateFlags.playerHypno && scene.playerAttack && scene.playerAttack.backfired){
        ...
        //hypno anim true
        scene.playerHypno = scene.add.sprite(scene.attacker.x, scene.attacker.y, "hypno_fx");
        scene.tweens.add({
    ...
    else if (scene.enemyState.hypno && !scene.stateFlags.enemyHypno && !scene.enemyAttack.backfired){
        ...
        //hypno anim true
        scene.enemyHypno = scene.add.sprite(scene.defender.x, scene.defender.y, "hypno_fx");
        scene.tweens.add({
    ...
    if (scene.playerState.sick && !scene.stateFlags.playerSick && scene.playerAttack && scene.playerAttack.backfired){
        ...
        // sick anim
        scene.playerSick = scene.add.sprite(scene.attacker.x, scene.attacker.y, "anim_sick_fx");
        scene.playerSick.anims.play("anim_sick_fx", true);
    }
    else if (scene.enemyState.sick && !scene.stateFlags.enemySick && !scene.enemyAttack.backfired){
        ...
        //sick anim\
        scene.enemySick = scene.add.sprite(scene.defender.x, scene.defender.y, "anim_sick_fx");
    ...
    if (scene.playerState.rage && !scene.stateFlags.playerRage){
        ...
        if (scene.stateFlags.playerSick){
            scene.stateFlags.playerSick = false;
            //remove sick anim
            scene.playerSick?.destroy();
        }
        if (scene.stateFlags.playerHypno){
...
function updateEnemyState(scene){
    if (scene.enemyAttack == null) {
        console.warn("Enemy attack is null, skipping state updates.");
        return;
    }

    if (scene.enemyState.hypno && !scene.stateFlags.enemyHypno && scene.enemyAttack.backfired){
        scene.stateFlags.enemyHypno = true;
        if (scene.stateFlags.enemyRage){
            scene.stateFlags.enemyRage = false;
            //remove rage anim
    ...
    if (scene.enemyState.sick && !scene.stateFlags.enemySick && scene.enemyAttack.backfired){
        scene.stateFlags.enemySick = true;
        if (scene.stateFlags.enemyRage){
            scene.stateFlags.enemyRage = false;
            //remove rage anim
    ...
    if (scene.enemyState.rage && !scene.stateFlags.enemyRage){
        ...
        if (scene.stateFlags.enemySick){
            scene.stateFlags.enemySick = false;
            //remove sick anim
            scene.enemySick?.destroy();
        }
        if (scene.stateFlags.enemyHypno){
